...
void tarea_A_code( void* taskParmPtr )
{
    ...
    UBaseType_t my_prio = uxTaskPriorityGet( 0 );   /* se podria haber usado uxTaskPriorityGet( task_handle_a ) */
                                                    /* devuele la prioridad de la tarea que le paso como parametro
    /* creo la tarea B con menor prioridad que la que esta corriendo actualmente
       No hay CC */
    res = xTaskCreate(
              ...
              configMINIMAL_STACK_SIZE*2,  /* tamaño del stack de cada tarea (words) */
              NULL,                        // Parametros de tarea
              my_prio - 2,                 /* le doy prioridad Prioridad IDLE + 2. 2 menos que la tarea actual (la A) */
              &task_handle_b               // Referencia a la tarea creada en el sistema
          );

    configASSERT( res == pdPASS );

    /* creo la tarea C con menor prioridad que la que esta corriendo actualmente
       No hay CC */
    res = xTaskCreate(
    ...
    /* creo la tarea D con menor prioridad que la que esta corriendo actualmente
       No hay CC */
    res = xTaskCreate(
              ...
              configMINIMAL_STACK_SIZE*2,  /* tamaño del stack de cada tarea (words) */
              NULL,                        // Parametros de tarea
              my_prio - 3,                 /* le doy prioridad Prioridad IDLE + 1. 3 menos que la tarea actual (la A) */
    ...
    vTaskSuspend( NULL );                   /* se podria haber usado vTaskSuspend( task_handle_a ) */

    blink_n_500( 2, LEDB );

    /* suspendo la tarea actual y se ejecuta la siguiente tarea en prioridad*/
